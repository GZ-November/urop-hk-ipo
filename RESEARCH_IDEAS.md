...
- **核心科学问题**：
  1. **财富让渡之谜**：在香港 IPO 市场中，发售价（IPO Offer Price）为何经常低于首日二级市场收盘价这一可观测的市场价格代理，从而产生巨额留存在桌面上的财富（Money Left on the Table, MLOT）？为何理性的控股股东与管理层愿意容忍本可通过更高发行价筹集的资金被让渡给获配投资者？需要强调：首日收盘价是市场定价结果，并不等同于不可观测的“真实内在价值”。
...
- **2026 Q1 样本微观现实**：
  - **有符号 MLOT 净额较大**：38 家样本按“首日收盘价减发售价”计算的有符号 MLOT 合计为 **256.64 亿港元**，单家平均 **6.75 亿港元**；破发公司的负值在该合计中作为抵减项保留；
  - **显性现金成本 vs 隐性抑价让渡**：同期 38 家公司支付给中介机构（保荐人、承销商、律所、审计师、公关打印）的显性现金上市费用（`Listing expenses`）总额为 **40.40 亿港元**（平均每家 1.06 亿港元）；
  - **6.35 倍的描述性比例**：有符号 MLOT 净额约为显性上市费用的 **6.35 倍**。该比例用于描述经济量级，不应在未完成因果识别前直接解释为发行人的确定性融资成本。
  - **首日表现两极分化**：Q1 首日破发率高达 **23.7%**（9 家破发），但其余 29 家股票首日大幅上冲（最高首日涨幅超 100%），印证了留足安全垫与过度挤干定价之间的生死分水岭。

### 6.4 机制深化一：反事实推演——“如果发行人不留钱在桌上，会发生什么？”
如果发行人坚持将发售价定在估值区间上端、尽量压缩预期首日回报，可能面对以下五类反事实成本。这些机制是待检验假说，而不是已经由当前 38 家样本证明的事实：

1. **簿记动量枯竭与上市夭折（Bookbuilding Freeze & Deal Collapse）**：
   - 国际配售路演中，如果定价未给投资者留下足够风险补偿，订单覆盖可能不足，发行人可能需要降价、缩减发行规模、延期或撤回。机构簿记需求目前并非本数据集可直接观察的变量，因此应以发行规模调整、定价修正、延期及撤回事件作为代理。
2. **孖展穿仓与首日踩踏式抛售（Day-1 Margin Call & Liquidation Cascades）**：
   - 2026 Q1 样本公开发售超额认购倍数平均约 **1,438 倍**、最高约 **5,297 倍**，显示零售需求极端集中。但超额认购不等于实际孖展杠杆；在尚未取得券商融资金额、融资比例和强平数据前，“孖展踩踏”只能作为待检验机制，不能当作既定事实。
3. **超额配股权（绿鞋）防线瞬间被击穿耗尽（Greenshoe Defense Depletion）**：
   - 超额配股权通常不超过基础发售规模的 15%，稳定价格经办人可在规则允许的稳定期内采取市场购入等行动。应收集实际稳定价格购股数量、价格区间、日期及绿鞋行使结果，检验支持能力是否与破发表现相关，而不预设其必然在首日耗尽。
4. **二级市场再融资报复性惩罚（Follow-on SEO Discount Punishment）**：
   - 如果 IPO 阶段的激进定价损害投资者回报，后续配售或供股可能需要提供更高折价。但折价幅度必须通过上市后 6/12/24 个月再融资事件数据计算，不能预先假定为 20%–30%。
5. **流动性枯竭与仙股化死循环（Post-IPO Liquidity Freeze & Zombie Stock Risk）**：
   - 激进定价可能与上市后成交额下降、价差扩大和机构覆盖不足相关。该机制应使用 20/60/126 个交易日的成交额、换手率、Amihud 非流动性及波动率检验；当前单一 6M 成交额比率不足以证明“流动性黑洞”。
...
1. **国有企业与央企（SOEs - SASAC 国资保值增值与防流失红线）**：
   - **待检验制度约束**：国资发行人可能面临更强的国有资产保值、审批及问责约束，但是否存在适用于每宗境外 IPO 的统一“不得低于每股净资产”硬规则需要逐案核对官方批复、估值备案与招股书披露。实证上应采集发行价/每股净资产、最终控制人层级和国资审批信息，而不是预设绝对底价。
...
3. **触碰港交所第 18A / 18C 章“法定最低上市市值门槛”的科技企业**：
   - **制度红线**：18A 生物科技公司上市时初始市值门槛为 **15 亿港元**。18C 在 2024 年 9 月 1 日至 2027 年 8 月 31 日的临时调整期内，已商业化公司门槛为 **40 亿港元**、未商业化公司为 **80 亿港元**；变量必须按各 IPO 申请及上市时适用的规则版本计算。
...
4. **超级科技独角兽霸权创始人（Superstar Tech Founders）**：
   - 明星创始人的议价力可能提高顶格定价概率，但“自负”不可直接观察。建议使用创始人投票权、CEO/董事长两职合一、基石覆盖率、承销团集中度与媒体关注度作为事前代理变量。
...
- **$H_{6b}$（存量老股转让与现金流权敏感性假说）**：发售中包含的老股转让比例（`Sale shares / Global offering shares`）越高，股东对真实现金退出价格越敏感，其留在桌面上的财富可能更小、定价更可能落在发售区间上限。**当前 2026 Q1 的 38 家样本该变量全部为 0，必须扩展历史样本后才可检验。**
- **$H_{6c}$（国资防流失刚性底线假说）**：受制于国资委每股净资产与防流失问责硬约束，国有企业（`SOE flag == 1`）的发售定价刚性显著高于非国企，其首日抑价率（Underpricing %）与留在桌面上的财富（MLOT）显著偏低，破发概率相对更高。
- **$H_{6d}$（制度性市值红线逼空假说）**：按发售价测算之总市值越接近上市时适用的第 18A 章（15 亿）或第 18C 章门槛（临时调整期为 40/80 亿），发行人的向下定价空间越小，其 MLOT 可能低于市值缓冲更充足的同行。
...
#### 模型 6.1：留在桌面上的财富决定方程（有符号变换与两部分模型）
破发公司的负 MLOT 是真实观察值而非删失值，因此不把它机械设为 0 后使用 Tobit。基准模型保留正负号，并将金额换算为百万港元后使用反双曲正弦变换：
$$\begin{aligned}
\operatorname{asinh}(\text{SignedMLOT}_i/10^6) = &\ \alpha_0 + \beta_1 \Delta P_i + \beta_2 \text{SecondaryRatio}_i + \beta_3 \text{SOE}_i + \beta_4 \text{CapThresholdProximity}_i \\
...
\end{aligned}$$
- **两部分稳健性检验**：第一部分估计 $\Pr(\text{MLOT}\le 0)$；第二部分仅在 $\text{MLOT}>0$ 的样本中估计 $\ln(1+\text{MLOT}/10^6)$。这样同时保留破发事件与正向财富让渡的经济含义。
...
- **子模型 6.3a：首日破发 Probit 模型**：
  $$\text{Prob}(\text{Day1Break}_i = 1) = \Phi\left(\lambda_0 + \lambda_1 \text{AtHigh}_i + \lambda_2 \Delta P_i + \lambda_3 \ln(1+\text{SubscriptionRatio}_i) + \lambda_4 \text{MechanismB}_i + \mathbf{W}_i \mathbf{\psi}\right)$$
  其中不再把由同一首日价格计算的 MLOT 放回破发方程，避免机械同源和结果变量泄漏。
...
| 18C 特专科技标识 | **Col 78** (`col_BV`) | `Chapter 18C flag` | 0/1 虚拟变量 | 按上市时规则版本匹配门槛；2024-09-01 至 2027-08-31 临时为 40/80 亿港元 |
...
| 零售超额认购倍数 | **Col 105** (`col_DA`)| `Subscription Ratio (times)` | 浮点数值 (倍数) | 公开发售需求代理变量（样本平均约 1,438x、最高约 5,297x）；不能直接等同于孖展杠杆 |
...
### 6.9 经济学直觉与学术边际贡献
1. **理论对话与香港制度场景**：本研究对接 Lowry et al. (2017) 的 IPO 定价谜题，拟利用香港主板 FINI 数字化结算改革、Mechanism A/B、基石配售及存量老股结构，区分纸面财富效应与真实现金退出激励。
2. **反事实机制的可检验化**：将簿记不足、稳定价格行动、上市后流动性与再融资折价转化为可收集的事件和结果变量，检验“留钱在桌面”是否具有流动性保险功能，而不是预先宣称该机制已经成立。
