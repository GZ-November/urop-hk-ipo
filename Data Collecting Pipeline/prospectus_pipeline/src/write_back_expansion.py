...
class WorkbookExpansionWriter:
    ...
    def __init__(self, book_path: Path = BOOK_PATH) -> None:
    ...
    def load_sources(self) -> None:
        ...
        # 1. 稳价事件
        stab_csv = OUT_MASTER / "stabilization_events.csv"
        if stab_csv.exists():
        ...
        # 2. 跨期收益
        h_csv = OUT_MASTER / "horizon_summary.csv"
        if h_csv.exists():
        ...
        d_csv = OUT_MASTER / "daily_market_panel.csv"
        if d_csv.exists():
        ...
        # 4. 解禁事件
        lk_csv = OUT_MASTER / "lockup_events.csv"
        if lk_csv.exists():
        ...
        # 5. 投资者关系统计
        inv_csv = OUT_MASTER / "investor_relational.csv"
        if inv_csv.exists():
        ...
        # 6. 承销辛迪加
        syn_csv = OUT_MASTER / "underwriter_relational.csv"
        if syn_csv.exists():
        ...
        # 7. Master 表
        im_csv = OUT_MASTER / "issuer_master.csv"
        if im_csv.exists():
            with im_csv.open("r", encoding="utf-8-sig") as fh:
                for r in csv.DictReader(fh):
                    self.master_data[r["stock_code"]] = r

    def get_cell_value(self, code: str, col_idx: int) -> tuple[Any, str]:
        """按列索引计算单元格写入值及格式。"""
    ...
    def write_expansion(self) -> int:
        ...
        with workbook_transaction(self.book_path, operation="academic_expansion") as wb:
            ...
            for r in range(2, 40):
                ...
                for col_idx, header, num_format, desc in EXPANSION_COLUMNS:
                    val, fmt = self.get_cell_value(code_str, col_idx)
                    ...
                    # 对齐与格式
                    if num_format in ("0.00%", "0.000", "#,##0", "0.000000"):
                        cell.alignment = DATA_ALIGN_RIGHT
                        cell.number_format = num_format
                    elif num_format in ("0", "yyyy-mm-dd"):
